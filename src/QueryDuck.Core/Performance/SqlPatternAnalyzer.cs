...
internal static partial class SqlPatternAnalyzer
{
    public sealed record SqlPatternFindings(
        ...
        IReadOnlyList<string> FromTables,
    ...
    public static SqlPatternFindings Analyze(string sql)
    {
        ...
        var joinTables = ExtractJoinTables(normalized);
        var fromTables = ExtractFromTables(normalized);
    ...
    private static IReadOnlyList<string> ExtractFromTables(string sql)
    {
        var matches = FromTableRegex().Matches(sql);
        return matches.Select(m => m.Groups[1].Value).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
    }
    ...
    [GeneratedRegex(@"\b(?:INNER|LEFT|RIGHT|FULL|CROSS)?\s*JOIN\s+((?:\w+\.)?\w+)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex JoinTableRegex();

    [GeneratedRegex(@"\b(?:FROM|UPDATE|INTO)\s+((?:\w+\.)?\w+)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
